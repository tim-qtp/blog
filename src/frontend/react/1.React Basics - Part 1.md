...
> JSX并不是标准的JS语法，它是JS的语法扩展，浏览器本身不能识别，需要通过==解析工具==做解析之后才能在浏览器中使用
...
// 函数组件
function Button(){
  return <button>click me mbti</button>
}
// 箭头函数组件
const Button = ()=>{
...
> 在React中状态被认为是只读的，应该始终`替换它而不是修改它`, 直接修改状态不能引发视图更新
...
![](https://qtp-1324720525.cos.ap-shanghai.myqcloud.com/blog/image-20250223150236639.png)
